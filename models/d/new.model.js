...
export const DataHubModel = {
  ...
  async create(data = {}) {
    ...
    const bucketId = data.bucketId || null;
    ...
    const sql = `
      INSERT INTO datahub_data (
        id, user_id, bucket_id, parent_id, name, type, content, file_size, mime_type, is_public, config, tags, sync_url, last_synced, created_at, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
    `;
  ...
  async getRootItems(userId, bucketId = null) {
    const sql = `
      SELECT 
        id,
        user_id,
        ...
        updated_at
      FROM datahub_data
      WHERE user_id = ? AND parent_id IS NULL ${bucketId ? 'AND bucket_id = ?' : ''}
      ORDER BY type DESC, name ASC
    `;
    
    const params = bucketId ? [userId, bucketId] : [userId];
  ...
  async togglePublicStatus(id, isPublic, userId = null) {
    const sql = `
      UPDATE datahub_data 
      SET is_public = ?, updated_at = NOW()
      WHERE id = ? ${userId ? 'AND user_id = ?' : ''}
